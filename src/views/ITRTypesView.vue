<template>
  <div class="p-4 md:p-6 bg-glacial min-h-screen">
    ...
    <div v-if="!projectStore.activeProject" class="text-center py-16">
      ...
      <p class="text-gray-600 mb-6">Select a project to manage its ITR types.</p>
    ...
    <template v-else>
      ...
      <div class="flex items-center justify-between mb-6 flex-wrap gap-3">
        <div>
          <h1 class="text-2xl font-bold text-gray-800">ITR Types</h1>
          <p class="text-sm text-gray-600">
            {{ projectStore.activeProject.name }} — {{ itrTypeStore.itrTypes.length }} type(s)
          </p>
        </div>
      ...
      <!-- ─── Loading ───────────────────────────────────────────────── -->
      <div v-if="itrTypeStore.loading && itrTypeStore.itrTypes.length === 0" class="text-center py-16">
        <div class="animate-spin rounded-full h-12 w-12 border-b-2 border-moss mx-auto"></div>
      </div>

      <!-- ─── DB Error ──────────────────────────────────────────────── -->
      <div v-else-if="itrTypeStore.error" class="py-6">
          ...
          <div class="flex gap-3">
            ...
            <div class="flex-1">
              ...
              <div class="text-sm text-yellow-700">
                Run <code class="bg-yellow-100 px-1 rounded">database/011_itr_types.sql</code> in
                <strong>Supabase Dashboard → SQL Editor</strong>, then click Retry.
              </div>
              <div class="mt-1 text-xs text-yellow-600 opacity-70">{{ itrTypeStore.error }}</div>
      ...
      <!-- ─── Empty state ───────────────────────────────────────────── -->
      <div v-else-if="itrTypeStore.itrTypes.length === 0" class="text-center py-16">
        <svg class="w-16 h-16 mx-auto mb-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        </svg>
        <h2 class="text-xl font-semibold mb-2 text-gray-800">No ITR Types Yet</h2>
        <p class="text-gray-600 mb-6">Click Add Type to get started.</p>
      </div>
        ...
        >
          ...
          <div class="w-20">
            <span
              ...
              :style="{ 
                backgroundColor: t.color ? t.color + '20' : '#81938A20',
                color: t.color ?? '#81938A'
              }"
            >{{ t.code }}</span>
          </div>

          <!-- Title -->
          <div class="flex-1 text-gray-800 font-medium">{{ t.title }}</div>
          ...
          <div class="w-16 flex justify-center">
            <svg v-if="t.is_active" class="w-5 h-5 text-green-600" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div class="w-20 flex justify-end gap-0 opacity-0 group-hover:opacity-100 transition-opacity">
            <button @click="openEdit(t)" title="Edit" class="p-1 text-gray-500 hover:text-moss transition-colors">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
              </svg>
            </button>
            <button @click="confirmDelete(t)" title="Delete" class="p-1 text-gray-500 hover:text-red-600 transition-colors">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="bg-white rounded-xl shadow-xl w-full max-w-md max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div class="flex items-center gap-2 px-5 py-4 border-b">
          ...
          <h3 class="text-lg font-semibold flex-1">
            {{ editTarget ? 'Edit ITR Type' : 'Add ITR Type' }}
          </h3>
        ...
        <div class="flex items-center justify-end gap-2 px-5 py-4 border-t bg-gray-50">
          ...
          >
            <svg v-if="itrTypeStore.loading" class="animate-spin h-4 w-4" fill="none" viewBox="0 0 24 24">
    ...
    <div v-if="deleteDialogOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div class="bg-white rounded-xl shadow-xl w-full max-w-sm">
        <div class="px-5 py-4">
          <h3 class="text-lg font-semibold mb-2">Delete ITR Type?</h3>
          <p class="text-gray-600">
            "<strong>{{ deleteTarget?.code }} — {{ deleteTarget?.title }}</strong>" will be removed.
            ITRs linked to this type will have their type cleared.
          </p>
        </div>
        <div class="flex items-center justify-end gap-2 px-5 py-4 border-t bg-gray-50">
          <button
...
import { useItrTypeStore, type ItrType } from '@/stores/itrTypeStore'
...
const editTarget       = ref<ItrType | null>(null)
const deleteDialogOpen = ref(false)
const deleteTarget     = ref<ItrType | null>(null)
...
const sortedTypes = computed(() =>
  [...itrTypeStore.itrTypes].sort((a, b) => a.sort_order - b.sort_order)
)
...
const resetForm = () => Object.assign(form, {
  code: '', title: '', color: '',
  sort_order: itrTypeStore.itrTypes.length + 1,
  is_active: true,
})

const openCreate = () => { editTarget.value = null; resetForm(); dialogOpen.value = true }

const openEdit = (t: ItrType) => {
  editTarget.value = t
  Object.assign(form, { code: t.code, title: t.title, color: t.color ?? '', sort_order: t.sort_order, is_active: t.is_active })
...
const saveType = async () => {
  ...
  if (editTarget.value) {
    const updated = await itrTypeStore.updateItrType(editTarget.value.id, payload)
    if (updated) { showSnack('ITR Type updated'); closeDialog() }
    else showSnack(itrTypeStore.error ?? 'Update failed', 'error')
  } else {
    const created = await itrTypeStore.createItrType({ ...payload, project_id: project.id })
    if (created) { showSnack(`"${created.code}" added`); closeDialog() }
    else showSnack(itrTypeStore.error ?? 'Create failed', 'error')
  }
}

// ── Delete ────────────────────────────────────────────────────────────────────

const confirmDelete = (t: ItrType) => { deleteTarget.value = t; deleteDialogOpen.value = true }

const doDelete = async () => {
  if (!deleteTarget.value) return
  const ok = await itrTypeStore.deleteItrType(deleteTarget.value.id)
  if (ok) showSnack('ITR Type deleted')
  else showSnack(itrTypeStore.error ?? 'Delete failed', 'error')
...
const loadForProject = async (projectId: string) => {
  itrTypeStore.clearItrTypes()
  await itrTypeStore.fetchItrTypes(projectId)
}
...
watch(() => projectStore.activeProjectId, async id => {
  ...
  else itrTypeStore.clearItrTypes()
})

onUnmounted(() => itrTypeStore.clearItrTypes())
</script>
